...
export default connect(Dogs, (store: Store<{ dogs: DogsSlice }>) => {
    ...
    slice.addReducer(
        fetchSampleImage,
        (state, imageUrl) => ({ ...state, breedSampleImageUrl: imageUrl }),
        "FETCH_SAMPLE_IMAGE",
    );
    slice.addReducer(
        getSampleImage,
        (state, breedName) => ({ ...state, selectedBreed: breedName }),
        "SELECT_BREED_NAME",
    );
    ...
    const actionMap: ActionMap<DogsProps> = {
    ...
    slice.destroyed.subscribe(() =>
        console.info("Dog slice got destroyed, all action/reducer pairs on this slice were removed"),
    );
